...
class ModelShippingMelhorenvios extends Model {
	function getQuote($address) {
		...
		$quote_data[$value['id']] = array(
				...
				'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('melhorenvios_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency']) . ' <b>entrega em até <span style="color:#019432;">'. $dias .'</span>'. $tdias.'</b>'
			);
		}
		}
		}
