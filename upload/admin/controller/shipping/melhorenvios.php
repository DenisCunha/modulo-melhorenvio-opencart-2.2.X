...
class ControllerShippingMelhorenvios extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/melhorenvios', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('shipping/melhorenvios', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('shipping/melhorenvios', $data));
	}
